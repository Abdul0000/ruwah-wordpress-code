...
$hero_image_html = '';
if ($hero_image_id) {
    $hero_image_html = wp_get_attachment_image($hero_image_id, 'full', false, ['loading' => 'eager', 'fetchpriority' => 'high', 'decoding' => 'async']);
} elseif ($hero) {
    $hero_image_html = $hero->get_image('full', ['loading' => 'eager', 'fetchpriority' => 'high']);
}
...
<?php
/* Home uses a standalone template, so remove WordPress' legacy site icon here too. */
remove_action('wp_head', 'wp_site_icon', 99);
wp_head();

/* Match Shop/direct pages: current Ruwah logo only, with a cache-busting favicon revision. */
$rwb_home_logo_id = (int) get_theme_mod('custom_logo', 0);
$rwb_home_logo_url = $rwb_home_logo_id ? wp_get_attachment_url($rwb_home_logo_id) : '';
if ($rwb_home_logo_url) {
    $rwb_home_favicon_url = add_query_arg('rwb-favicon', '20260820-home2', $rwb_home_logo_url);
    echo '<link rel="icon" type="image/png" href="' . esc_url($rwb_home_favicon_url) . '">' . "\n";
...
.rwb-reference-home-v5 .rwb-ref-header:not(.compact) .rwb-brand .custom-logo{width:auto;max-width:205px;max-height:92px;filter:none!important;object-fit:contain}
.rwb-reference-home-v5 .rwb-ref-header:not(.compact) .rwb-brand .custom-logo-link{filter:drop-shadow(0 1px 8px rgba(247,243,233,.52))}
...
.rwb-reference-home-v5 .rwb-ref-hero{height:calc(100vh - 40px);min-height:720px;display:grid;place-items:center;background:#514645;color:#f7f3e9}
.rwb-reference-home-v5 .rwb-ref-hero-wash{background:radial-gradient(ellipse at 50% 49%,rgba(32,23,35,.52) 0 18%,rgba(32,23,35,.34) 38%,rgba(32,23,35,.12) 62%,transparent 80%),linear-gradient(90deg,rgba(18,14,19,.28),rgba(18,14,19,.10) 45%,rgba(18,14,19,.30)),linear-gradient(0deg,rgba(8,7,9,.26),transparent 52%,rgba(8,7,9,.12))!important}
.rwb-reference-home-v5 .rwb-ref-hero-copy{width:940px;max-width:90vw;padding:122px 24px 0;text-align:center;color:#f7f3e9!important;text-shadow:0 2px 24px rgba(8,6,8,.48)!important}
.rwb-reference-home-v5 .rwb-ref-hero-copy .rwb-ref-kicker{display:inline-flex!important;align-items:center;justify-content:center;width:max-content;margin:0 auto!important;padding:7px 11px;border:1px solid rgba(255,255,255,.48);background:rgba(247,243,233,.92);color:#735790!important;font-size:13px;font-weight:700;line-height:1;letter-spacing:.10em;text-shadow:none!important;text-transform:uppercase}
.rwb-reference-home-v5 .rwb-ref-hero-copy h1{margin:20px 0 0;color:#fbf8f0!important;font-family:var(--serif,'DM Serif Display',Georgia,serif)!important;font-size:clamp(62px,4vw,78px);font-weight:400!important;line-height:.95;letter-spacing:-.032em!important}
.rwb-reference-home-v5 .rwb-ref-hero-copy h2{max-width:900px;margin:12px auto 0;color:#eee5f7!important;font-family:var(--sans,Inter,Arial,sans-serif);font-size:clamp(36px,2.8vw,52px);font-weight:500;line-height:1.02;letter-spacing:-.035em}
.rwb-reference-home-v5 .rwb-ref-hero-copy>p:not(.rwb-ref-kicker){margin:22px auto 0;color:#fffaf2!important;font-size:23px;font-weight:500;line-height:1.25;letter-spacing:-.015em}
.rwb-reference-home-v5 .rwb-ref-hero-btn{width:228px;min-width:228px;height:64px;min-height:64px;margin-top:34px;padding:0 24px;border:1px solid rgba(255,255,255,.42)!important;background:#876cad!important;color:#fff!important;font-family:var(--sans,Inter,Arial,sans-serif)!important;font-size:13px!important;font-weight:700!important;line-height:1;text-shadow:none;text-transform:uppercase!important;letter-spacing:.08em!important}
.rwb-reference-home-v5 .rwb-ref-hero-btn:hover{background:#70578f!important;color:#fff!important}
...
<section class="rwb-ref-hero" aria-label="Featured formula">
    <div class="rwb-ref-hero-media" aria-hidden="true"><div class="rwb-ref-hero-bg"><?php echo wp_kses_post($hero_image_html); ?></div><div class="rwb-ref-hero-focus"><?php echo wp_kses_post($hero_image_html); ?></div></div>
