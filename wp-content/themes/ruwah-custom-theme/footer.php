...
.rwb-support-line{display:block;max-width:520px;margin-top:4px;line-height:1.5;text-transform:none!important;letter-spacing:0!important}
...
<nav id="rwb-contact-dock" class="rwb-contact-dock" aria-label="Contact Ruwah Beauty">
  ...
  <a class="rwb-contact-dock-item rwb-contact-dock-item--gmail" href="mailto:rawahbeauty783@gmail.com" aria-label="Email Ruwah Beauty at rawahbeauty783@gmail.com" title="Email: rawahbeauty783@gmail.com">
    <span class="rwb-contact-dock-label">Email</span>
...
<?php wp_footer(); ?>
<script id="rwb-footer-production-readiness">
(()=>{'use strict';
const links={Facebook:'https://www.facebook.com/share/1BNAdjWpYW/',Instagram:'https://www.instagram.com/rawah.beauty?utm_source=qr&igsh=ZjMzazdrNmk1aTVt',TikTok:'https://vt.tiktok.com/ZSX6WqwS2/'};
document.querySelectorAll('.rwb-dieux-footer-socials span[aria-label]').forEach(span=>{if(!span.getAttribute('role'))span.setAttribute('role','img');if(span.closest('a'))return;const label=span.getAttribute('aria-label')||'';const href=links[label];if(!href)return;const a=document.createElement('a');a.href=href;a.target='_blank';a.rel='noopener noreferrer';a.className='rwb-dieux-social-link';a.setAttribute('aria-label',label+' — Ruwah Beauty');a.title='Follow Ruwah Beauty on '+label;span.parentNode.insertBefore(a,span);a.appendChild(span);});
document.querySelectorAll('button').forEach(button=>{if((button.textContent||'').trim().toLowerCase()==='initiate me')button.textContent='Join Ruwah Notes';});
document.querySelectorAll('input[type="email"]').forEach(input=>{const p=(input.getAttribute('placeholder')||'').trim().toLowerCase();if(p==='my email address is')input.setAttribute('placeholder','Email address');if(!input.getAttribute('aria-label'))input.setAttribute('aria-label','Email address');});
document.querySelectorAll('.rwb-dieux-payments span').forEach(el=>{const t=(el.textContent||'').trim().toLowerCase();if(t.includes('payfast'))el.textContent='COD ONLY';else if(t==='secure')el.textContent='ONLINE: SOON';});
const meta=document.querySelector('.rwb-dieux-footer-meta');if(meta&&!meta.querySelector('.rwb-support-line')){const line=document.createElement('span');line.className='rwb-support-line';line.textContent='Support: rawahbeauty783@gmail.com · WhatsApp +92 371 3923279 · Response target: within 2 business days';meta.appendChild(line);}
})();
</script>
</body></html>