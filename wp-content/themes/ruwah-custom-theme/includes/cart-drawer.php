...
/**
 * Read-only current-session hydration for cached storefront HTML.
 * No nonce is required because this endpoint never mutates the cart; it only
 * returns the WooCommerce cart attached to the request's existing session.
 */
function rwb_cart_drawer_refresh_state(): void {
    if (! rwb_cart_drawer_ensure_cart()) {
        wp_send_json_error(['message' => 'Cart is unavailable.'], 400);
    }
    rwb_cart_drawer_ajax_response();
}
add_action('wp_ajax_rwb_cart_drawer_refresh_state', 'rwb_cart_drawer_refresh_state');
add_action('wp_ajax_nopriv_rwb_cart_drawer_refresh_state', 'rwb_cart_drawer_refresh_state');
