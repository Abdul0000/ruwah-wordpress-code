...
add_action('wp_enqueue_scripts',function(){wp_enqueue_style('astra-parent',get_template_directory_uri().'/style.css',[],wp_get_theme('astra')->get('Version'));$base=get_stylesheet_directory().'/style.css';$polish=get_stylesheet_directory().'/assets/css/component-polish.css';$js=get_stylesheet_directory().'/theme.js';wp_enqueue_style('ruwa-rich',get_stylesheet_directory_uri().'/style.css',['astra-parent'],file_exists($base)?filemtime($base):'24.0.0');wp_enqueue_style('ruwa-component-polish',get_stylesheet_directory_uri().'/assets/css/component-polish.css',['ruwa-rich'],file_exists($polish)?filemtime($polish):'25.0.0');wp_enqueue_script('ruwa-rich',get_stylesheet_directory_uri().'/theme.js',[],file_exists($js)?filemtime($js):'25.0.0',true);},30);
add_filter('body_class',function($c){$c[]='ruwa-rich-theme';$c[]='ruwa-polished-components';return $c;});
